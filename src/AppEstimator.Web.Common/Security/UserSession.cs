...
namespace AppEstimator.Web.Common.Security
{
    public class UserSession : IWebUserSession
    {
        ...
        public long AppEstimatorUserId
        {
            get 
            {
                var currentPrincipal = (ClaimsPrincipal)HttpContext.Current.User;                
                var userId = currentPrincipal.Claims.Where(c => c.Type == Constants.AppEstimatorClaimTypes.UserIdentifier)
                                                   .Select(c => c.Value)
                                                   .SingleOrDefault();
